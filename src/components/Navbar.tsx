...
function ChevronIcon({ isOpen }: { isOpen?: boolean }) {
  return (
    <motion.svg
      ...
      className="h-[8px] w-[14px] shrink-0 fill-current text-white/90 transition-colors group-hover:text-white"
      viewBox="0 0 15 9"
      fill="currentColor"
  ...
  return (
    <header className="relative z-40 w-full pt-[32px] sm:pt-[45px]">
      <div className="mx-auto flex max-w-[1440px] items-center justify-between px-6 sm:px-10 lg:px-[99px]">
        ...
        >
          <span className="font-[family-name:var(--font-turret)] text-[32px] sm:text-[36px] font-bold tracking-tight text-white transition-opacity group-hover:opacity-90">
            Ether
          </span>
        </motion.a>
            ...
            {NAV_ITEMS.map((item, idx) => {
              ...
              return (
                ...
                >
                  <a
                    href={item.href}
                    className={`group inline-flex items-center gap-2 font-[family-name:var(--font-work)] ${item.fontSize} font-normal text-white/90 transition-colors hover:text-white`}
                  >
                    <span>{item.name}</span>
                    <ChevronIcon isOpen={isHovered} />
                  </a>

                  {/* Dropdown Menu Indicator/Surface */}
                  <AnimatePresence>
                    {isHovered && (
                      <motion.div
                        ...
                        className="absolute left-0 top-full mt-2 w-48 rounded-xl border border-white/10 bg-zinc-950/90 p-2 shadow-2xl backdrop-blur-xl"
                      >
        ...
        <div className="md:hidden">
          <button
            ...
            className="inline-flex h-10 w-10 items-center justify-center rounded-lg border border-white/10 bg-white/5 text-white transition-colors hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-purple-500/50"
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="overflow-hidden border-b border-white/10 bg-black/95 px-6 py-4 backdrop-blur-2xl md:hidden"
          >
              ...
              {NAV_ITEMS.map((item) => (
                <li key={item.name}>
                  <a
                    ...
                    className="flex items-center justify-between py-2 font-[family-name:var(--font-work)] text-lg text-white/90 transition-colors hover:text-white"
                  >
                    <span>{item.name}</span>
                    <ChevronIcon />
                  </a>
